...
- **Rogue is driven by the persisted cron `errorCount`, which must be re-fetched**: `fetchBotSettings()` has a 30s staleness guard, so call it from refresh timers alongside `fetchBotStats()` — otherwise rogue stays frozen at page-load state (legacy re-scanned the cron table every 10s).
