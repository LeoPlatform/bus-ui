...
export const { handle, signIn, signOut } = SvelteKitAuth({
	providers: [
		Cognito({
			issuer: `https://cognito-idp.${AWS_REGION}.amazonaws.com/${AUTH_COGNITO_USER_POOL_ID}`,
			authorization: {
				params: {
					scope: 'openid email profile'
				}
		...
		// 	name: 'Custom',
		// 	type: 'oidc',
		// 	issuer: 'https://staging-core.dsco.io/tools/index/dw-auth-token',
		// }
		//TODO: this will probably need to be added as part of the stack. 
		//TODO: any new OIDC providers that aren't natively set up as supported providers in AWS IDentity Pools (Google, Cognito) will need to be created
		// as new OIDC Identity providers
		// GitHub
	],
	secret: AUTH_SECRET,
	trustHost: true,
	...
	callbacks: {
		async session({ session, token }) {
            ...
            if(LOCAL && AWS_SESSION_TOKEN) {
				...
				try {
					...
					const credentials = await getAwsCredentials(token.id_token as string, token.provider);
		...
		async jwt({ token, account }) {
			if (account) {
                ...
                try {
                    const refreshedTokens = await refreshAccessToken(token.refresh_token, token.provider!);
...
async function getAwsCredentials(idToken: string, provider: string = 'cognito') {
    ...
    if(provider === 'google') {
        ...
        logins[`cognito-idp.${AWS_REGION}.amazonaws.com/${AUTH_COGNITO_USER_POOL_ID}`] = idToken
    } else if(provider === 'github') {
		//TODO: re-enable once stack is built
