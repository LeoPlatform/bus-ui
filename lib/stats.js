...
module.exports = function(event, callback) {
	...
	console.log("Requested Timestamp:", request_timestamp.format(), range.count, numberOfPeriods)
	...
	function statsProcessor(done) {
		...
		console.log("query:", period, moment(start).format(), start, moment(end).format(), end);
		leo.aws.dynamodb.query({
			...
			.catch(err => done(err))
			.then(bucketsStats => {
				console.log(period, bucketsStats.LastEvaluatedKey, bucketsStats.ConsumedCapacity, bucketsStats.Items.length)
				var out = {};
