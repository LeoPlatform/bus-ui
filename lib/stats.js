...
// Take the response and merge it together
	...
	function parallelQuery(queries, opts, mergeFn) {
		...
		//TODO: let this define a possible 'limit' but still handle all that
		// if (queries.length > 30) {
		// 	// chunk the queries up
		// 	//TODO: actually do this rather than returning
		// 	console.log("WHOA way too many queries man: ", queries.length);
		// 	return; 
		// }
		...
		for (const [index, query] of queries.entries()) {
			...
			if (statsCache[key] && isBucketClosed) {
				...
				requests.push(dynamodb.query(query, opts).then(data => {
					// Cache results when we actually have data
